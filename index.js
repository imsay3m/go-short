const config = {
	...
	theme: "",//Homepage theme, use the empty value for default theme.
...
<body>
  ...
  <a href="https://github.com/imsay3m/go-short/" target="_self">Fork me on GitHub</a>
</body>`
...
async function is_url_safe(url) {
	...
	let raw = JSON.stringify({ "client": { "clientId": "go-short", "clientVersion": "1.0.4" }, "threatInfo": { "threatTypes": ["MALWARE", "SOCIAL_ENGINEERING", "POTENTIALLY_HARMFUL_APPLICATION", "UNWANTED_SOFTWARE"], "platformTypes": ["ANY_PLATFORM"], "threatEntryTypes": ["URL"], "threatEntries": [{ "url": url }] } });
...
async function handleRequest(request) {
	...
	if (request.method === "POST") {
		...
		if (!await checkURL(req["url"])) {
			return new Response(`{"status":500,"key":": Error: Url is invalid Please Put https:// before the url."}`, {
	...
	if (!path) {
		...
		const html = await fetch("https://imsay3m.github.io/go-short/" + config.theme + "/index.html")
	...
	if (location) {
		if (config.safe_browsing_api_key) {
			if (!(await is_url_safe(location))) {
				let warning_page = await fetch("https://imsay3m.github.io/go-short/safe-browsing.html")
				warning_page = await warning_page.text()
				warning_page = warning_page.replace(/{Replace}/gm, location)
				new Response(warning_page, {
					headers: {
						"content-type": "text/html;charset=UTF-8",
					},
				})
		...
		if (config.no_ref == "on") {
			let no_ref = await fetch("https://imsay3m.github.io/go-short/no-ref.html")
